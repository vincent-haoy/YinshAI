...
class BoardArea():
    ...
    def update(self, state, resources):
        #Update rings "won" and previously removed from play.
        for i in [0,1]:
            for j in range(state.rings_won[i]):
                piece_name = 'RING_0' if i==0 else 'RING_1'
                self.root.create_image(SCORE_POS[0]+i*SCORE_SEP[0], SCORE_POS[1]+j*SCORE_SEP[1], 
                                       image=resources[piece_name], tags='piece')
        #Update rings and counters in play.
        for y in range(11):
            for x in range(11):
                piece = state.board[y,x]
                if piece not in [EMPTY, ILLEGAL]:
                    x1, y1 = x, y+x*0.5 #A simple skew transform to convert coordinates to a non-orthogonal board.
                    piece_name = 'CNTR_0' if piece==CNTR_0 else 'CNTR_1' if piece==CNTR_1 else \
                                 'RING_0' if piece==RING_0 else 'RING_1' if piece==RING_1 else None
                    self.root.create_image(BOARD_POS[0]+x1*PIECE_SEP[0], BOARD_POS[1]+y1*PIECE_SEP[1], 
                                           image=resources[piece_name], tags='piece')
...
class GUIDisplayer(Displayer):
    ...
    def InitDisplayer(self, runner):
        ...
        self.root['bg']='black'
        self.root.title("YINSH ------ COMP90054 AI Planning for Autonomy")
        self.root.tk.call('wm', 'iconphoto', self.root._w, tkinter.PhotoImage(file='Yinsh/resources/ICON_0.png'))
        ...
        self.resources = {'table': tkinter.PhotoImage(file="Yinsh/resources/table.png" ).subsample(int(1/s)),
                          'CNTR_0':tkinter.PhotoImage(file="Yinsh/resources/CNTR_0.png").subsample(int(1/s)),
                          'CNTR_1':tkinter.PhotoImage(file="Yinsh/resources/CNTR_1.png").subsample(int(1/s)),
                          'RING_0':tkinter.PhotoImage(file="Yinsh/resources/RING_0.png").subsample(int(1/s)),
                          'RING_1':tkinter.PhotoImage(file="Yinsh/resources/RING_1.png").subsample(int(1/s))}
        ...
        self.agent_title_var.set(f'{runner.agents_namelist[0]} (Teal) vs. {runner.agents_namelist[1]} (Magenta)')
        self.agent_titles = make_label(self.canvas, textvariable=self.agent_title_var, x=TITL_POS[0], 
                                       y=TITL_POS[1], h=30*s, w=700*s, font=('times', int(10*s)), bg='black', fg='white')
        

        #Generate scoreboard in separate window.
        self.sb_window = tkinter.Toplevel(self.root)
        self.sb_window.title("Yinsh ------ Activity Log")
        self.sb_window.tk.call('wm', 'iconphoto', self.sb_window._w, tkinter.PhotoImage(file='Yinsh/resources/ICON_1.png'))
...
#Class to represent games as text when the GUI is not requested.
