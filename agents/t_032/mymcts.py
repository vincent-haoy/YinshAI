...
class myAgent(Agent):
    ...
    def SelectAction(self,actions,game_state):
        ...
        tree = MCTSTREE(2,30,self.game_rule,1000)
        ...
        while time.time() - starttime <= THRESHOLD:
            ...
            result = greedysimulator(simulatestate,self.game_rule,self.id)   
...
def greedysimulator(state, gamerule, role):
    ...
    while isnotEndstate(state) and sequencefound != 0:
        ...
        scoredaction = QvalueEvaluator(avaliabelaction,state)
...
class MCTSTREE:
    def __init__(self,depth, ExpandnodeInPly, gamerule, Nodetoexpand, exploration=False):
        self.depth = depth
    ...
    def _recursivelyBuildtree(self,node,depth,id):
        state = node.state
        if depth > 0:
            node.getaction(state,self.gamerule)
            node.sortaction()
            #building successors
...
class MCTSNODE:
    ...
    def getaction(self,state,gamerule):
        ...
        scored_action = QvalueEvaluator(allactions,state)
