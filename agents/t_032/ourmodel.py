...
class YinshGameRule(GameRule):
    ...
    def numberofControledTokens(self,game_state,agent_id):
        ringspos = game_state.ring_pos[agent_id]
        totalcontrolled = 0
        regex = re.compile(f"{EMPTY}*[{CNTR_0}{CNTR_1}]+{EMPTY}"+'{1}')
        for pos in ringspos:
            for line in ['v','h','d']:
                ...
                for p in positions:
                    ...
                    p_str  = ('').join([str(game_state.board[i]) for i in passed[1:]])
                    if re.fullmatch(regex, p_str):
                        totalcontrolled += len([a for a in p_str if a != str(EMPTY)])
            ...
            for y in range(11): #For all positions on the board,
                for x in range(11):
                    if game_state.board[(y,x)]==EMPTY: #If this position is empty,
                        actions.append({'type': 'place ring', 'place pos':(y,x),'moveby':agent_id}) #Generate a 'place ring' action.
        
        elif not game_state.counters_left: #Agent has to pass if there are no counters left to play.
            return [{'type':'pass','moveby':agent_id}]
                        ...
                        if sequences[agent_id]:
                            for r_pos in game_state.ring_pos[agent_id]:
                                actions.append({'type':'place, move, remove', 'place pos':ring_pos, 'move pos':pos, \
                                                'remove pos':r_pos, 'sequences': sequences,'sequence remove': agent_id,'moveby':agent_id})
                        elif sequences[0 if agent_id==1 else 1]:    
                            for r_pos in game_state.ring_pos[agent_id]:
                                actions.append({'type':'place and move', 'place pos':ring_pos, 'move pos':pos, \
                                                'sequences': sequences,'sequence remove':(1 - agent_id),'moveby':agent_id})                            
                        else:
                            actions.append({'type': 'place and move', 'place pos':ring_pos, 'move pos':pos,'moveby':agent_id})
...
# END FILE -----------------------------------------------------------------------------------------------------------#
    def ReverstateByactions(self,actions,state):
    
        agent_id = actions['moveby']
        if actions['type'] =='place ring':
            state.board[actions['place pos']] = EMPTY
            state.rings_to_place += 1

        elif(actions['type'] == 'pass'):
            return
            ...
            ring_pos = actions['place pos']
            move_pos = actions['move pos']
            start_pos,end_pos = (ring_pos, move_pos)
            state.counters_left += 1
            # if we remove a ring, we have to place it back 
            if actions['type'] == 'place, move, remove':                
                remove_pos = actions['remove pos']
                state.board[remove_pos] = ring
                state.ring_pos[agent_id].append(remove_pos)    
                state.rings_won[agent_id] -= 1   
                for sequence in actions['sequences'][agent_id]:
                    state.board[sequence] = cntr

            
            #if we take others' 5 markers, put them back
            if actions.get('sequence remove',-99) != -99:    
                if actions['type'] == 'place and move':
                    state.rings_won[1-agent_id] -= 1
                    rmovedcntr = CNTR_0 if agent_id==1 else CNTR_1
                    for sequence in actions['sequences'][1-agent_id]:
                        state.board[sequence] = rmovedcntr


            # else just simply undo the movement
            state.board[ring_pos] = ring
            state.board[move_pos] = EMPTY
            state.ring_pos[agent_id].remove(move_pos)
            state.ring_pos[agent_id].append(ring_pos)
            [self.flip(state.board, p) for p in self.positionsPassed(start_pos, end_pos)]