...
class Q_value_class(object):
    ...
    def update(self,next_Q,act_idx):
        delta = self.cal_delta(next_Q,act_idx)
        self.Q_value[act_idx] = self.Q_value[act_idx]+self.alpha*delta
        if self.Q_value[act_idx] > self.max_Q_value:
            self.max_Q_value = self.Q_value[act_idx]
